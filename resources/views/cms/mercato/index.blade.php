...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header border-0 py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">{{__('cms.mercatos')}}</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm"></span>
        </h3>
        @can('Create-Mercato')
        <div class="card-toolbar">
            <a href="{{route('mercatos.create')}}"
                class="btn btn-info font-weight-bolder font-size-sm">{{__('cms.mercatos')}}</a>
        </div>
        @endcan
    </div>
    <!--end::Header-->
            ...
            <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_2">
                <thead>
                    <tr class="text-uppercase">
                        ...
                        <th style="min-width: 150px">{{__('cms.salary')}}</th>

                        @canany(['Update-Mercato','Create-Mercato','Delete-Mercato'])
                        <th class="pr-0 text-right" style="min-width: 160px">{{__('cms.actions')}}</th>
                        @endcanany
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data as $mercato)
                    <tr>
                        <td class="pl-0">
                            <div class="symbol symbol-50 symbol-light mr-4">
                                <span class="symbol-label">
                                    <img src="{{Storage::url($mercato->poster)}}" class="customImage h-75 align-self-end"
                                        alt="">
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="pl-0">
                            <a href="#"
                                class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$mercato->translations->first()?->title
                                ?? ''}}</a>
                        </td>

                        <td class="pl-0">
                            <a href="#"
                                class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$mercato->type
                                ?? ''}}</a>
                        </td>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <table class="table">
                                                ...
                                                <tbody>
                                                    @foreach ($mercato->translations as $translation)
                                                    <tr>
                                                        <td>
                                                            <span
                                                        ...
                                                        <td class="pr-0 text-right">
                                                            @can('Update-Mercato')
                                                            <a href="{{route('mercato-translations.edit',$translation->id)}}"
                        ...
                        <td class="pl-0">
                            <a href="#"
                                class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$mercato->form
                                ?? ''}}</a>
                        </td>
                        ...
                        <td class="pl-0">
                            <a href="#"
                                class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$mercato->to
                                ?? ''}} </a>
                        </td>

                      <td class="pl-0">
                        <a href="#" class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$mercato->salary
                            ?? ''}} $</a>
                    </td>

                    <td class="pl-0">
                        <a href="#" class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$mercato->start_date
                            ?? ''}} </a>
                    </td>

                    <td class="pl-0">
                        <a href="#" class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{$mercato->end_date
                            ?? ''}} </a>
                    </td>
                        ...
                        <td class="pr-0 text-right">
                            @can('Create-Mercato')
                            <a href="{{route('mercato-translations.create',$mercato->id)}}"
                                class="btn btn-icon btn-light btn-hover-primary btn-sm">
                            ...
                            @endcan
                            @can('Delete-Mercato')
                            <a href="#" onclick="performSportsDestroy('{{$mercato->id}}', this)"
                                class="btn btn-icon btn-light btn-hover-primary btn-sm">
                                <span class="svg-icon svg-icon-md svg-icon-primary">
                                    <!--begin::Svg Icon | path:assets/media/svg/icons/General/Trash.svg-->
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
...
<script>
    function performSportsDestroy(id,reference) {
        confirmDestroy('/cms/admin/mercatos', id, reference);
    }
    function performTranslationDestroy(id,reference) {
        confirmDestroy('/cms/admin/mercatos/translations', id, reference);
    }
</script>
@endsection
